import {
  NormalizedError,
  normalizeError,
} from './domain/normalizeError.ts'
import { RemoteError } from './errors/remote.error.ts'
import { RpcTimeoutError } from './errors/rpcTimeout.error.ts'
import {
  ExecuteProps,
...
import { WILL_NOT_HANDLE } from './constants.ts'
      ...
      if (additionalHandlers.length) {
        const tasks = additionalHandlers.map((handler: any) =>
          handler(
            ...
            msg,
          ).catch((err: Error | typeof WILL_NOT_HANDLE) => {
